...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/03 13:08:16 by ivloisy           #+#    #+#             */
/*   Updated: 2021/02/03 13:08:17 by ivloisy          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_ceil(t_cub *cub, int x)
{
	...
	{
		ft_pixel_put(cub->img, x, y, add_shade(((double)y
				/ (double)(cub->h / 2)), get_rgb(&(cub->c_c))));
		y++;
	}
}
...
static void	draw_floor(t_cub *cub, int x)
{
	...
	while (y < cub->h)
	{
		ft_pixel_put(cub->img, x, y, add_shade((1.0 - ((double)(y - cub->h / 2)
				/ (double)(cub->h / 2))), get_rgb(&(cub->f_c))));
		y++;
	}
}
	...
	while (y < cub->ray.end)
	{
		...
		if (cub->ray.cardinal == 'E')
			color = cub->tex_ea->addr[cub->tex_ea->w * cub->ray.tex_y
					+ cub->ray.tex_x];
		ft_pixel_put(cub->img, x, y, add_shade((cub->ray.dist / 10), color));
		y++;
	}
}
...
int			display(t_cub *cub)
{
	...
	if (draw_sprites(cub, z) != SUCCESS)
		return (cub->error);
	if (cub->m_x * 10 < cub->w && cub->m_y * 10 < cub->h)
		minimap(cub);
	if (cub->w > 30 && cub->h > 30)
		mir(cub);
	return (SUCCESS);
}
