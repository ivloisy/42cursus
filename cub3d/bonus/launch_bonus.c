...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/03 13:11:13 by ivloisy           #+#    #+#             */
/*   Updated: 2021/02/03 13:11:14 by ivloisy          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			get_tex(t_cub *cub)
{
	...
	if (cub->f_path == 1)
	{
		if ((cub->tex_f = new_texture(cub, cub->f_p)) == NULL)
			return (cub->error = TEX_F_ERROR);
	}
	if (cub->c_path == 1)
	{
		if ((cub->tex_c = new_texture(cub, cub->c_p)) == NULL)
			return (cub->error = TEX_C_ERROR);
	}
	return (SUCCESS);
}
