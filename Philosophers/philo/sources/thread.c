...
/*   Updated: 2021/11/18 16:58:20 by ivloisy          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	assign_fork(t_data *data, t_ph *ph)
{
	if (ph->id == data->arg.number_of_philosophers)
		ph->l_fork = &data->arg.fork[0];
	else
		ph->l_fork = &data->arg.fork[ph->id];
	ph->r_fork = &data->arg.fork[(ph->id - 1)
...
static int	init_ph(t_data *data)
{
	...
	while (i < data->arg.number_of_philosophers)
	{
		...
		if (pthread_mutex_init(&data->ph[i].n, NULL) != 0)
			return (0);
		if (pthread_mutex_init(&data->ph[i].meal, NULL) != 0)
			return (0);
		i++;
	}
	return (1);
